...
import com.sina.crowclub.network.Parse.dao.ChannelDao;
import com.sina.crowclub.network.Parse.db.SQLHelper;
...
public class ChannelManage {
    ...
    private static final String TABLE_NAME = SQLHelper.TABLE_CHANNEL;
    /**
     * 默认的用户选择频道列表
     * */
    ...
    static {
        ...
        defaultOtherChannels.add(new ChannelItem(19, "在线直播", 12, 0));
    }
    ...
    public void deleteAllChannel() {
        channelDao.clearFeedTable(TABLE_NAME);
    }
    /**
     * 获取其他的频道
    ...
    public List<ChannelItem> getUserChannel() {
        Object cacheList = channelDao.listCache(SQLHelper.SELECTED + "= ?",new String[] { "1" },TABLE_NAME);
    ...
    public List<ChannelItem> getOtherChannel() {
        Object cacheList = channelDao.listCache(SQLHelper.SELECTED + "= ?" ,new String[] { "0" },TABLE_NAME);
        List<ChannelItem> list = new ArrayList<ChannelItem>();
        if (cacheList != null && !((List) cacheList).isEmpty()){
            List<Map<String, String>> maplist = (List) cacheList;
            int count = maplist.size();
    ...
    public void saveUserChannel(List<ChannelItem> userList) {
        for (int i = 0; i < userList.size(); i++) {
            ...
            channelDao.addCache(channelItem,TABLE_NAME);
        }
    }
