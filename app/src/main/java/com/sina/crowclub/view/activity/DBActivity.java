...
import com.sina.crowclub.database.CommonDB;
    ...
    View.OnClickListener
{
    ...
    private void doGetReadDB(){
        ThreadPool.execute(new Runnable() {
            ...
            public void run() {
                ...
                final List<ChannelItem> list = MApplication.mCommonDao.getSortNewsItemList();
    ...
    private void doSelectNewsItem(final int id){
        ThreadPool.execute(new Runnable() {
            @Override
            public void run() {
                ChannelItem item = MApplication.mCommonDao.getNewsById(id);
    ...
    private void doSortNewsItems(){
        ThreadPool.execute(new Runnable() {
            @Override
            public void run() {
               List<ChannelItem> list = MApplication.mCommonDao.getSortNewsItemList();
               if(list != null && list.size() > 0){
